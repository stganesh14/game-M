...
setInterval(function () {
    if(timeShow.innerHTML != " "){
        time -= 1;
       timeShow.innerHTML = time + " Seconds left"
    }
},1000)
...
function comapre(touchElement){
    const boxes = document.querySelectorAll('.box');
    for(let i =0;i<boxes.length;i++){
        const boxPostion = boxes[i].getBoundingClientRect();
...
function display(){
    ...
    scoreHeading.innerHTML = "Congrats!!!</br></br></br>" + "Score : " + score ;
...
function checkvictory(){
    ...
    game.appendChild(parentDiv);
    const childDiv = document.createElement('div');
    childDiv.classList.add('box');
    parentDiv.appendChild(childDiv);
    parentDiv.style.position = "absolute";
    parentDiv.style.top = generateRandomNumber() + "%";
    parentDiv.style.left = generateRandomNumber() + "%";
